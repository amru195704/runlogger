...
class _MyRunPageState extends State<MyRunPage> {
    ...
    Timer.periodic(
      ...
      (Timer timer) async {
        ...
        if (mounted == true) {
          ...
          _spdGraphList =
              Gps.getGpsLogGraphData(0, _minGraphX, _maxGraphX, _xMaxDefault);
          // 標高のmin/max/data取得　(スケール５)
          _minAlt = convGraphValue(Gps.getAltMin(), false, 5);
          _maxAlt = convGraphValue(Gps.getAltMax(), true, 5);
          _altGraphList =
              Gps.getGpsLogGraphData(1, _minGraphX, _maxGraphX, _xMaxDefault);
          // 表示文字を取得
  ...
  LineChartData get altlineChartData {
    ...
    var linechart = LineChartData(
      ...
      titlesData: FlTitlesData(
        topTitles: AxisTitles(
          axisNameWidget: Text("標高($_altStr)"),
          axisNameSize: 35.0,
        ),
        rightTitles:
            const AxisTitles(sideTitles: SideTitles(showTitles: false)),
    ...
    return Scaffold(
      body: Column(children: [
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
          TextButton(
            style: styl1,
            onPressed: () => {_graphType = GraphType.first, print("<<が押されたよ")},
            child: const Text("<<"),
          ),
          TextButton(
            style: styl1,
            onPressed: () => {_graphType = GraphType.all, print("allが押されたよ")},
            child: const Text("all"),
          ),
          TextButton(
            style: styl1,
            onPressed: () => {_graphType = GraphType.last, print(">>が押されたよ")},
            child: const Text(">>"),
          ),
        ]),
      ]),
